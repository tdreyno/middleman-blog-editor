...
App.Article = DS.Model.extend({
  ...
  published: function(key, value) {
    ...
    if (arguments.length === 1) {
      ...
      if (!row) {
        row = App.Frontmatter.createRecord({
          id: this.get('id') + '-published',
          key: 'published',
          value: '' + value + '',
          valueBoolean: value,
          article: this
        ...
        this.get('frontmatters').pushObject(row);
      }
  ...
  root: Ember.Route.extend({
    ...
    index: Ember.Route.extend({
      ...
      createArticle: function(router) {
        ...
        var publishedFront = App.Frontmatter.createRecord({
          id: 'un-named-published',
          key: 'published',
          value: 'true',
          valueBoolean: true
        });
        newModel.get('frontmatters').pushObject(publishedFront);
        var titleFront = App.Frontmatter.createRecord({
          id: 'un-named-title',
          key: 'title',
          value: 'New Article'
        });
        newModel.get('frontmatters').pushObject(titleFront);
        router.transitionTo('root.edit', newModel);
      },
