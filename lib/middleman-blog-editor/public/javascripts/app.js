...
App.EpicEditorView = Ember.View.extend({
  value: "",
  ...
  didInsertElement: function() {
    ...
    var opts = {
      ...
      basePath: '/' + ROOT_DIR + '/javascripts/vendor/epiceditor',
    ...
    editor.load(function() {
      editor.importFile('blog-post', self.get('value'));
    });
    ...
    editor.on("save", function() {
      self.set("value", editor.exportFile());
    });
...
App.Article = DS.Model.extend({
  ...
  isMarkdown: function() {
    var engine = this.get('engine');
    return (engine === 'md') || (engine === 'markdown');
  }.property('engine'),

  WYSIWYGable: function() {
    return this.get('isERB') || this.get('isMarkdown');
  }.property('isERB', 'isMarkdown'),
  ...
  tags: function(key, value) {
    if (arguments.length === 1) {
      ...
      this.setFrontmatter('tags', value);
      return value;
    }
  }.property('frontmatter'),
...
App.Router = Ember.Router.extend({
  ...
  root: Ember.Route.extend({
    ...
    index: Ember.Route.extend({
      ...
      createArticleERb: function(router) {
        router.transitionTo('root.create', { engine: 'erb' });
      },

      createArticleMarkdown: function(router, moar, params) {
        router.transitionTo('root.create', { engine: 'md' });
      },
